...
if (isset($_POST['user_id']) AND isset($_POST['product_configuration_id']) AND isset($_POST['comment']) AND isset($_POST['datetime_ordered']) AND preg_match('#(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})$#', $_POST['datetime_ordered']))
{
	...
	if ($user_connected->userExists() AND $product_configuration_displayed->productConfigurationExists())
	{
		...
		$user_connected->addOrder($product_configuration_displayed->getProductConfigurationId(), $_POST['datetime_ordered'], $_POST['comment']);
